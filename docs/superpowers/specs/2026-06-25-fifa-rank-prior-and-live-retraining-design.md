...
**Per-team regularization strength:** `l2ByTeam[id] = baseL2 × (100 / max(effectiveMatchCount[id], 1))` — a team at exactly 100 effective matches gets the unmodified `baseL2`; below 100, regularization strengthens proportionally; above 100, it weakens. Applied to every team (48 or not) — a non-World-Cup team with very little data still benefits from being pulled toward *its* prior (zero), it just doesn't get a FIFA-rank-based one.
